...
•	انواع کانولوشن: 
o	Full: خروجی با طول N+M−1N + M - 1N+M−1 (کانولوشن کامل).
o	Same: خروجی هم‌اندازه با سیگنال اصلی.
o	Valid: خروجی بدون اعمال لبه‌های ناشی از Padding (طول خروجی max⁡(M,N)−min⁡(M,N)+1\max(M, N) - \min(M, N) + 1max(M,N)−min(M,N)+1).
...
1.	به صفحه دانلود رسمی Intel IPP مراجعه کنید.
